...
export async function getSession(request: NextRequest): Promise<UserSession | null> {
  ...
  try {
    ...
    return { id: payload.id, email: payload.email, name: payload.name, role: payload.role, image: payload.image }
  } catch {
    return null
  }
}
...
export async function requireAdmin(request: NextRequest): Promise<UserSession | null> {
